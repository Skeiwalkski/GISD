...
dir.create("Outfiles/2023_v0", showWarnings=T)
dir.create("Outfiles/2023_v0/Bund", showWarnings=T)
dir.create("Outfiles/2023_v0/Other", showWarnings=T)
dir.create("Outfiles/2023_v0/Stata", showWarnings=T)
setwd(home)
getwd()
```
...
load_dataset = function(sheet) {
  suppressMessages(
    read_excel("Data/Referenz/Erzeugte Referenzen/Referenz_1998_2020.xlsx", sheet = sheet, na = "NA")
    ...
    # (Angleichung an NUTS-Reform 2011)

if (!file.exists("Data/Referenz/Erzeugte Referenzen/Referenz_1998_2020.xlsx")) {
  source("Data/Referenz/Generate_Reference_2020.R") #Aktelle Referenz generieren wenn noch nicht vorhanden
}
...
Gemeindeverbaende_INKAR <- load_dataset("Gemeindeverbände") %>% 
  na.omit() %>% 
  select(GVBKennziffer = gvb_kz_lang,
         "Name des Gemeindeverbands" = gvbname)

Kreise_INKAR <- load_dataset("KRS") %>%
  mutate(kkz= as.numeric(kkz), flaeche = as.numeric(flaeche))
...
write_rds(id_dataset, paste0("Outfiles/2023_v0/ID_dataset_2023"))
```
...
Basedata <- Kreise_INKAR %>% select(Kennziffer=kkz) %>% mutate(Jahr=2020)
...
for(file in inputdataset){
  suppressMessages(myimport <- read_excel(paste0("Data/INKAR_1998_2020/",file), skip = 1, sheet = "Daten"))
...
Basedata_Gemeindeverbandsebene <- Basedata %>% 
  select(Kennziffer, Jahr, Arbeitslosigkeit, Beschaeftigtenquote,
         ErwerbsfaehigeBevoelkerung, Einkommensteuer) %>%   
  gather(key,value,3:5) %>%
  filter(!is.na(value)) %>%
  spread(key,value) %>% 
  filter(Jahr>=1998) %>% 
  rename("Gemeindeverband"=Kennziffer)
...
Basedata_Kreisebene <- Basedata %>%
  select(krs15 = Kennziffer ,Jahr, listofdeterminants) %>% 
...
write_dta(Gemeinden_ohne_Bevoelkerung, paste0("Outfiles/2023_v0/Gemeinden_ohne_Bevoelkerung.dta"))
...
write_dta(Workfile, paste0("Outfiles/2023_v0/Workfile_vorab.dta"))
...
write_dta(stata_data, paste0("Outfiles/2023_v0/Stata/Impdata_check_2019.dta"))
rm(stata_data)

# RDS-Datensatz rausschreiben
write_rds(Impdata.imputed, paste0("Outfiles/2023_v0/Impdata_check.rds"))
...
save(GISD_Komponents, file="Outfiles/2023_v0/GISD_Komponents.RData")
...
write_rds(Resultdataset, paste0("Outfiles/2023_v0/Resultdataset.rds"))
write_dta(Resultdataset, paste0("Outfiles/2023_v0/Stata/Resultdataset.dta"))
```
...
for(mykennziffer in exportlist$Kennziffern) {
  ...
  dir.create("Outfiles/2023_v0/", showWarnings=F)
  dir.create("Outfiles/2023_v0/Bund/", showWarnings=F)
  mydata <- outputdata.agg %>% ungroup() %>% select(mykennziffer, gisd_score, gisd_5, gisd_10, gisd_k, myname, year)
  write.csv(mydata, paste0("Outfiles/2023_v0/Bund/GISD_Bund_",mylabel,".csv"), row.names=FALSE, fileEncoding="UTF-8")
  ...
  write_dta(mydata, paste0("Outfiles/2023_v0/Bund/GISD_Bund_",mylabel,"_long.dta"))
  ...
  dir.create("Outfiles/2023_v0/Bundesland", showWarnings=F)  
  for(myland in ListeBula) {
    ...
    write.csv(mydata.bula, paste0("Outfiles/2023_v0/Bundesland/GISD_",myland,"_",mylabel,".csv"), row.names=FALSE, fileEncoding="UTF-8")
    ...
    write_dta(mydata.bula, paste0("Outfiles/2023_v0/Bundesland/GISD_",myland,"_",mylabel,".dta"))
  }
  }  
}
...
for (mykennziffer in c("PLZ2","PLZ3","PLZ4","PLZ5")) {
  ...
  dir.create( paste0("Outfiles/2023_v0"), showWarnings=F) 
  dir.create( paste0("Outfiles/2023_v0/Bund/"), showWarnings=F) 
