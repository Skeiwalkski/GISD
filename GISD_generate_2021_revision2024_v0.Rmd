...
Die Indikatoren werden den einzelnen Teildimensionen des GISD zugewiesen. Datenjahre vor 2000 fallen nicht mit rein.
...
Results_raw <- Impdata %>%
  mutate(TS_Arbeitswelt = c(predict(TS_Arbeitswelt.pca, newdata = Impdata)),
         TS_Einkommen = c(predict(TS_Einkommen.pca , newdata = Impdata)),
         TS_Bildung = c(predict(TS_Bildung.pca , newdata = Impdata)))

rm(Impdata)

# Übersicht über Teildimensionen
summary(Results_raw %>% select(TS_Arbeitswelt,
                               TS_Einkommen,
                               TS_Bildung))
...
if (cor(Results_raw$Arbeitslosigkeit, Results_raw$TS_Bildung) < 0) {
  Results_raw$TS_Bildung <- Results_raw$TS_Bildung*-1
}
if (cor(Results_raw$Arbeitslosigkeit, Results_raw$TS_Arbeitswelt) < 0) {
  Results_raw$TS_Arbeitswelt <- Results_raw$TS_Arbeitswelt*-1
}
if (cor(Results_raw$Arbeitslosigkeit, Results_raw$TS_Einkommen) < 0) {
  Results_raw$TS_Einkommen <- Results_raw$TS_Einkommen*-1
}
...
# Normieren
Results_nrm <- Results_raw %>%
  group_by(Jahr) %>%
  mutate(
    # Teildimensionen normieren (neu: jahresweise statt über gesamte Zeitreihe)
    TS_Arbeitswelt_nrm = rescale(TS_Arbeitswelt),
    TS_Einkommen_nrm = rescale(TS_Einkommen),
    TS_Bildung_nrm = rescale(TS_Bildung),
  ...
  select(Gemeindekennziffer,
         ...
         Bevoelkerung,
         Bundesland,
         TS_Arbeitswelt_raw = TS_Arbeitswelt,
         TS_Einkommen_raw = TS_Einkommen,
         TS_Bildung_raw = TS_Bildung,
         contains("_nrm"),
         GISD_Score)

# Übersicht über Gesamt- und Teilscores nach Normierung
summary(Results_nrm %>% select(TS_Arbeitswelt_nrm,
                               TS_Einkommen_nrm,
                               TS_Bildung_nrm,
                               GISD_Score))
...
write_rds(Results_nrm, paste0(check_dir,"Results_nrm.rds"))

# # Stata-Kopie der rohen Scores für internes Verzeichnis
# write_dta(Results_raw, paste0(check_dir,"Results_raw.dta"))
...
# # Ausschreiben des normierten Scores zur Überprüfung
# write_rds(Results_nrm, paste0(outfiles_dir,"Results_nrm.rds"))

### Notiz: Vielleicht obsolet, check noch mal Ausreißer Skript
# # R-Kopie der rohen Scores für internes Verzeichnis
...
## Vorbereitung
# Teilscores aufbereiten (Basis: rohe Teilscores vor Normierung)
Scores_raw_long <- Results_nrm %>% 
...
rm(ausreisser_a, ausreisser_a_imp1,
   ...
   Results_nrm, Results_adj, Results_imp1, Results_imp2,
