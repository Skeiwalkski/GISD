...
Impdata.imputed <- Impdata.imputed %>%
  mutate(G8 = case_when(Kreis < 2000 & Jahr >= 2016 & Jahr <= 2028 ~ 1,
                                ...
                                Kreis > 15999 ~ 1),
         TH = ifelse(Jahr < 2004 & Kreis > 15999, 1, 0))
Impdata.imputed$G8[is.na(Impdata.imputed$G8)] = 0
Impdata.imputed$TH[is.na(Impdata.imputed$TH)] = 0
...
adj_G8 <- function(data,outcome_name){
  mydata   <- data %>%
    ...
    select(Gemeindekennziffer, Jahr, G8, TH, "Outcome"=paste(outcome_name)) %>% 
    mutate(MEAN=mean(Outcome , na.rm=T)) %>% ungroup()
  
  mymodell <- lm(Outcome ~
                  I(Jahr*Jahr*MEAN) + I(Jahr*MEAN) + G8 + TH, data = mydata  , na.action="na.exclude")
  
  mydata %>% select(Outcome, G8, TH) %>% mutate(pred_var = predict(mymodell, newdata = mydata)) %>%
    mutate(Outcome=ifelse(G8 == 1 | TH == 1,pred_var,Outcome)) %>% 
...
adj_G8_jahr <- function(data,outcome_name){
  mydata   <- data %>%
    group_by(Gemeindekennziffer) %>% 
    select(Gemeindekennziffer, Jahr, G8_jahr, SN_KA, TH, "Outcome"=paste(outcome_name)) %>% 
    mutate(MEAN=mean(Outcome , na.rm=T)) %>% ungroup()
  
    mymodell <- lm(Outcome ~
                  I(Jahr*Jahr*MEAN) + I(Jahr*MEAN) + G8_jahr + SN_KA + TH, data = mydata  , na.action="na.exclude")
  
  
    mydata %>% mutate(coef = summary(mymodell)$coefficients[4,1], coef_SH = summary(mymodell)$coefficients[5,1], coef_TH = summary(mymodell)$coefficients[6,1]) %>%
      mutate(Outcome = ifelse(G8_jahr == 1, Outcome - coef, Outcome), Outcome = ifelse(SN_KA == 1, Outcome - coef_SH, Outcome), Outcome = ifelse(TH == 1, Outcome - coef_TH, Outcome)) %>%
      pull(Outcome)
}
...
```{r Gedeckelte Beschäftigungsquote}
...
TS_Bildung_adj <- Impdata.imputed %>% dplyr::select(BeschaeftigtemitakadAbschluss,BeschaeftigteohneAbschluss,SchulabgaengerohneAbschluss_adj) 
...
summary(Resultdataset %>% dplyr::select(TS_Arbeitswelt, TS_Einkommen, TS_Bildung_adj))
descs <- stat.desc(Resultdataset[, -5])

# Korrelationen überprüfen
Resultdataset %>% dplyr::select(Arbeitslosigkeit,TS_Arbeitswelt,TS_Einkommen,TS_Bildung_adj)  %>% cor( use="pairwise.complete.obs")  
...
# GISD
Resultdataset$GISD_Score <- Resultdataset$TS_Arbeitswelt+Resultdataset$TS_Einkommen+Resultdataset$TS_Bildung_adj
...
# Result
summary(Resultdataset %>% select(TS_Arbeitswelt,TS_Einkommen,TS_Bildung_adj,GISD_Score))
str(Resultdataset %>% select(TS_Arbeitswelt,TS_Einkommen,TS_Bildung_adj,GISD_Score))
