...
# Pipes:  1. neben der Kennziffer, die einen anderen Namen bekommt wird das Jahr und die Variablenliste ausgew??hlt
#         2. drei Variablen werden aus der Auswahl ausgeschlossen
#         3. die Kreisvariable wird in Kreis umbenannt, weil im n??chsten Schritt Kreisinfos an die Gemeinden angespielt werden

# Join different levels
# Nun werden die Daten bezogen auf die Ebenen gemergt
# Dazu wird erstmal ein Leerdatensatz im Longformat erstellt, der F??lle f??r alle Gemeinden f??r jedes Jahr generiert
Workfile <- as.data.frame(expand.grid("Kennziffer"=Gemeinden_INKAR %>% pull(Kennziffer),"Jahr"=seq(min(Basedata$Jahr):max(Basedata$Jahr)) + min(Basedata$Jahr)-1, stringsAsFactors = F)) %>% mutate(Kreiskennziffer=substr(Kennziffer, 1, nchar(Kennziffer)-3)) %>% as_tibble() %>%
  ...
  arrange(Gemeindekennziffer,Jahr) %>% # Join Metadata
   left_join(. , Basedata_Kreisebene,by=c("Kreis","Jahr")) %>% # Hier wird ??ber Kreis gematched
...
#     pull erzeugt einen Vektor f??r die Variablenwerte von Kennziffer aus dem Datensatz
#     + min(...) wird zu der Sequenz von Jahren aus dem Basedata addiert (1 bis X) damit auch Jahreswerte weitergeben werden[ist das n??tig?]
#     stringAsFactors sorgt daf??r, dass die Kennziffern nicht als Factors sondern als Strings geladen werden und es damit keine Probleme bei der Weiterverarbeitung gibt
# 2. mutate generiert eine Kreiskennziffer
# 3. as_tibble erzeugt einen tibble, damit left_join genutzt werden kann
# 4. erstes left_join spielt die Gemeindedaten ??ber Kennziffer an, das geht so, weil Gemeinden_INKAR als tibble gespeichert ist
# 5. select, w??hlt die inhaltlichen Variablen aus, und ??ndert die Variablennamen
...
# [Probleme mit Datensatz: Besch??ftigte mit Abschluss = ohne Abschluss; Negativer Wert bei der Einkommenssteuer]
...
Es gibt noch einige Probleme bei der Auswahl der Indikatoren, die erst sp??ter zum Tragen kommen. Insbesondere der Bildungsindikator Schulabg??nger ohne Abschluss macht Probleme, weil der nicht mit dem Anteil Besch??ftigter mit akademischem Bildungsabschluss korreliert. 
Eine Betrachtung des alternativen Indikators Schulabg??nger mit Hochschulreife zeigt, dass dieser besser mit dem Anteil Besch??ftigter mit akademischem Bildungsabschluss korreliert, aber daf??r recht stark negativ mit dem ANteil der Beschdftigten ohne Abschluss. Das ldsst den Schluss zu, dass in Regionen in denen auch ohne Abschluss eine Perspektive besteht 



## III.Imputation fehlender Werte

Das bisherige Imputationsmodell nutzt Arbeitslosigkeit als Pr??diktoren. In den neuen??? Daten (2016) fehlen f??r diese Variable 6 Werte.
Der Einfachheit halber werden diese interpoliert. 

```{r Vereinzelte Missings auf den Imputationsvariablen interpolieren}
# Anzahl der Missings f??r die Indikatoren, die f??r das Imputationsmodell ben??tigt werden
...
# F??lle betrachten: Beispiel 5154028 
...
# Interpolation der fehlenden Werte ??ber die Zeitreihe (Mittelwert: Vorjahr, Nachjahr)
# sehr hd_liches Coding!! weil ein einfaches replace von Missings nicht m??glich ist
...
```{r}
# Anzahl der Missings |ber die Indikatoren
...
# Variablenliste f|r die Regressionsimputation wird erstellt
# das betrifft alle Variablen, au_er die im angebenen Vektor
# letztere sind frei von Missings und kvnnen im Imputationsmodell genutzt werden 
...
# Einige Missings basierten auf Gebietsstdnden ohne Bevvlkerung, diese sind entfernt 
# Damit kdme auch die Einkommenssteuer als Prddiktor im Imputationsmodell in Frage
...
# Hier wird eine Funktion generiert, die im Datensatz (data) fehlende Daten f|r ausgewdhlte Variablen (outcome_name) imputiert
# 1. zundchst werden Mittelwerte f|r das Outcome (siehe select) jeweils f|r die Gemeinde generiert, d.h. |ber alle Wellen aggregiert
...
#    wdhrend der vorherige Befehl (mymodell) die Koeffizienten generiert, werden nun auf Basis dieses Modells predictions generiert, 
#    und zwar auch f|r Fdlle mit Missing auf den Outcomes
# 4. fehlende Werte in den Outcomes werden durch Werte auf der Variable Imputed ersetzt
# 5. F|r einige Fdlle erzeugt die prediction unplausible Werte (negative Outcomes), diese werden auf 0 gesetzt
# 6. pull kreiert einen Vektor (hier Variable Outcome), die im ndchsten Befehl verwendet wird
...
# Percentile f|r ausgewdhlte Variablen bilden
...
# f|r den Vergleich der Ergebnisse wird zundchst ein Datensatz f|r die Variablenauswahl der Revision 2019 generiert
...
# Check dieser Lvsung f|r das 2014er Sample 
...
# Faktorenanalyse basierend auf Hauptkomponentenanalyse f|r jede der drei Subscalen
...
TS_Arbeitswelt.pca
	# Option retx erzeugt rotierte Lvsung

head(TS_Arbeitswelt.pca$sdev)
# nur die erste Komponente mit Eigenwert |ber 1
	# (prcomp gibt standardmd_ig Sdev statt Varianz aus)
plot(TS_Arbeitswelt.pca)
	# screeplot - bei nur drei Variablen wird ein Balkendiagramm angezeigt
TS_Arbeitswelt.pca
# die Faktorladungen der drei Hauptkomponenten f|r Arbeitswelt 
...
# die Option rank erlaubt die Beschrdnkung der ANzahl an Komponenten (Faktoren)
TS_Arbeitswelt.pca
plot(TS_Arbeitswelt.pca)

# PCA f|r die Einkommensdimension
...
# f|r die Bildung deutet die Analyse eher auf zwei Komponenten hin
# die Faktorladung f|r SchulabgaengerohneAbschluss ist auf dem ersten Faktor schwach, 
# die Faktorladung f|r BeschaeftigtemitakadAbschluss auf dem zweiten
# es wird die Komponente ausgewdhlt, bei der Beschaeftigte mit akad Abschluss positiv korreliert und 
...
# F|r den JoHM Beitrag wird die Bildungsdimension mit den zwei Indikatoren generiert
...
# Es wurde au_erdem eine Komponentenanalyse mit allen vier Bildungsindikatoren durchgef|hrt. 
...
# Matrix der "Faktorladungen" f|r die Bildungsdimension erstellen
...
# auch f|r die Teildimension Bildung werden Zeilenvektoren eingebunden
...
# die |berfl|ssigen Zeilennamen werden gestrichen
colnames(GISD_Komponents) <- c("Variable","Dimension","Anteil","Score")
# aussagekrdftige Spaltennamen vergeben
...
# Korrelationen erneut |berpr|fen
Resultdataset %>% dplyr::select(Arbeitslosigkeit,TS_Arbeitswelt,TS_Einkommen,TS_Bildung) %>% cor( use="pairwise.complete.obs")
