...
adj_G8 <- function(data,outcome_name){
  mydata   <- data %>%
    ...
    select(Gemeindekennziffer, Jahr, G8, "Outcome"=paste(outcome_name)) %>% 
    mutate(MEAN=mean(Outcome , na.rm=T)) %>% ungroup()
  
  mymodell <- lm(Outcome ~
                  I(Jahr*Jahr*MEAN) + I(Jahr*MEAN) + G8, data = mydata  , na.action="na.exclude")
  
  mydata %>% select(Outcome, G8) %>% mutate(pred_var = predict(mymodell, newdata = mydata)) %>%
    mutate(Outcome=ifelse(G8 == 1,pred_var,Outcome)) %>% 
...
adj_G8_jahr <- function(data,outcome_name){
  mydata   <- data %>%
    group_by(Gemeindekennziffer) %>% 
    select(Gemeindekennziffer, Jahr, G8_jahr, "Outcome"=paste(outcome_name)) %>% 
    mutate(MEAN=mean(Outcome , na.rm=T)) %>% ungroup()
  
    mymodell <- lm(Outcome ~
                  I(Jahr*Jahr*MEAN) + I(Jahr*MEAN) + G8_jahr, data = mydata  , na.action="na.exclude")
  
  
    mydata %>% mutate(coef = summary(mymodell)$coefficients[4,1]) %>%
      mutate(Outcome = ifelse(G8_jahr == 1, Outcome - coef, Outcome)) %>%
      pull(Outcome)
}
