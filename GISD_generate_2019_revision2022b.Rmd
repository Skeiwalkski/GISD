...
dir.create("Outfiles/2022b", showWarnings=T)
dir.create("Outfiles/2022b/Bund", showWarnings=T)
dir.create("Outfiles/2022b/Other", showWarnings=T)
dir.create("Outfiles/2022b/Stata", showWarnings=T)
setwd(home)
getwd()
```
...
ZunÃ¤chst wird ein Datensatz generiert in dem den kleinsten regionalen Einheiten (Gemeinden) alle Ã¼bergeordneten regionalen Einheiten und deren Regionalkennziffern zugeordnet werden. Datenquelle ist die Gebietsstandsreferenz von Destatis Stand 31.12.2017.
Es wird insbesondere geprÃ¼ft, ob die Referenzdaten Missings auf den Regionalkennziffern oder den Namen der GebietsstÃ¤nde aufweisen.
An diesen ID-Datensatz werden spÃ¤ter die Indikatoren angespielt. 
...
print_missings = function(data) {
  ...
  df}
# Eine Funktion die spÃ¤ter hÃ¤ufiger verwendet wird. ErklÃ¤rung der Befehle:
...
# 2. Wenn es im Datensatz fehlende Werte gibt, werden die dazugehÃ¶rigen 
...
# Eine Funktion die spÃ¤ter hÃ¤ufiger verwendet wird. Sie dient dazu die einzelnen 
# Sheets aus Excel einzulesen. AuÃerdem werden Warnmeldungen beim Laden der 
# Daten unterdrÃ¼ckt. Zum Aufrufen der Funktion muss der Name des gewÃ¼nschten
...
# 3. Rename von zwei Variablen; " um Leerzeichen zu berÃ¼cksichtigen 

Gemeindeverbaende_INKAR <- load_dataset("GemeindeverbÃ¤nde") %>% 
  na.omit() %>% 
  select(GVBKennziffer=gvb19,"Name des Gemeindeverbands"=gvb19name)
# Das ganze nochmal fÃ¼r GemeindeverbÃ¤nde  
# Pipes: 
# 1. Wenn es fehlende Werte gibt, wird man hierdurch benachrichtigt
# 2. Missings herausfiltern
# 3. Nur die Variablen gvb17 und Name des Gemeindeverbands ausgewÃ¤hlt
...
# 1. Wenn es fehlende Werte gibt, wird man hierdurch benachrichtigt
# 2. Missings herausfiltern
# 3. Neue Variable generieren, die die Kreisvariable auf FÃ¼nfsteller reduzieren
...
id_dataset <- Gemeinden_INKAR %>% 
              select(Gemeindekennziffer=Kennziffer,"Name der Gemeinde"=gem19name,GVBKennziffer, "BevÃ¶lkerung") %>% 
...
write_rds(id_dataset, paste0("Outfiles/2022b/ID_dataset_2019"))
```

## II. Erzeugen eines Datensatzes mit Kennziffern als ID unabhÃ¤ngig von der Ebene 

In diesem Code-Abschnitt werden die INKAR-Daten zu den Indikatoren in einem Datensatz zusammengefÃ¼hrt. Die Information fÃ¼r die Indikatoren, die fÃ¼r die Berechnung des GISD verwendet werden, liegt auf unterschiedlichen Ebenen vor. Die Faktorenanalysen sollen spÃ¤ter auf Gemeindeebene durchgefÃ¼hrt werden, weshalb Information der Kreisebene an alle Gemeinden dieser Kreise angespielt wird. Percentile des Indexes kÃ¶nnen so spÃ¤ter fÃ¼r jede regionale Ebene separat berechnet werden. Datenbasis sind die INKAR-Daten der jeweiligen Indikatoren im Excel-Format, die zu jeder Revision aus der INKAR-Datenbank heruntergeladen werden. Tabelle \@ref(tab:indicators) stellt die Indikatoren dar.

```{r Indikatoren einlesen und mit dem ID-Datensatz ZusammefÃ¼hren, message=FALSE}
...
inputdataset <- list.files("Data/INKAR_1998_2019/") # Variablenliste der Dateinamen im Ordner
...
# 3. Die zweite und dritte Zeile werden gelÃ¶scht
...
# rm.na entfert Zeilen, bei denen der "Value" fehlt; -"Kennziffer" sorgt dafÃ¼r, dass jeder Kennziffer mehrere Jahre zugeordnet werden
# 5. von innen nach auÃen 
# 5.1 das innere strsplit(file, "_") teilt den Filenamen inkl. Dateiendung beim "_"; mit [[1]][2] wird das zweite Element davon ausgewÃ¤hlt
# 5.3 das Ã¤uÃere strsplit teilt dies dann beim ".", sodass nur noch der Dateiname Ã¼brig bleibt, der mit [[1]][1] ausgewÃ¤hlt wird
# 5.5 names(import)[3] nimmt diesen Dateinamen als Variablennamen fÃ¼r die dritte Spalte
...
# Hinweis: fÃ¼r die Gemeindeebene wird kein Basedata-Datensatz erstell, da es keine Infos auf der Gemeindeebene gibt

# Join different levels
# Nun werden die Daten bezogen auf die Ebenen gemergt
# Dazu wird erstmal ein Leerdatensatz im Longformat erstellt, der FÃ¤lle fÃ¼r alle Gemeinden fÃ¼r jedes Jahr generiert
Workfile <- as.data.frame(expand.grid("Kennziffer"=Gemeinden_INKAR %>% pull(Kennziffer),"Jahr"=seq(min(Basedata$Jahr):max(Basedata$Jahr)) + min(Basedata$Jahr)-1)) %>% mutate(Kreiskennziffer=floor(as.numeric(Kennziffer)/1000)) %>% as_tibble() %>%
   ...
   select(Gemeindekennziffer="Kennziffer",Kreis="Kreiskennziffer",Gemeindeverband=GVBKennziffer,Jahr,Bevoelkerung="BevÃ¶lkerung") %>% mutate(Gemeindeverband=as.numeric(Gemeindeverband), Bevoelkerung=as.numeric(Bevoelkerung)) %>% 
...
#     + min(...) wird zu der Sequenz von Jahren aus dem Basedata addiert (1 bis X) damit auch Jahreswerte weitergeben werden[ist das nÃ¶tig?]
#     stringAsFactors sorgt dafÃ¼r, dass die Kennziffern nicht als Factors sondern als Strings geladen werden und es damit keine Probleme bei der Weiterverarbeitung gibt
...
##Erstellen eines Datensatzes mit BevÃ¶lkerungslosen Gemeinden und Entfernen dieser fÃ¼r die bevorstehende Imputation und die Faktoranalyse
Gemeinden_ohne_Bevoelkerung <- Workfile %>% filter(Bevoelkerung==0)
write_dta(Gemeinden_ohne_Bevoelkerung, paste0("Outfiles/2022b/Gemeinden_ohne_Bevoelkerung.dta"))

#Berechnung der Anteile fÃ¼r BeschÃ¤ftigte ohne Abschluss, BeschÃ¤ftigte mit akad. Abschluss und Entfernen SV BeschÃ¤ftigte (total)
...
#Berechnung der tatsÃ¤chlichen Arbeitslosenquote und Entfernen ErwerbsfÃ¤hige BevÃ¶lkerung
...
##Ausschreiben eines Vorab-Workfiles zur ÃberprÃ¼fung der Arbeitslosenquote
Workfile <- Workfile %>% filter(Bevoelkerung>0)
write_dta(Workfile, paste0("Outfiles/2022b/Workfile_vorab.dta"))

# Stata-Datensatz rausschreiben
#write_dta(Workfile, paste0("S:/OE/FG28/205 Regionale Unterschiede/GISD/PlausibilitÃ¤tschecks/workfile.dta"))
...
Es gibt noch einige Probleme bei der Auswahl der Indikatoren, die erst spÃ¤ter zum Tragen kommen. Insbesondere der Bildungsindikator SchulabgÃ¤nger ohne Abschluss macht Probleme, weil der nicht mit dem Anteil BeschÃ¤ftigter mit akademischem Bildungsabschluss korreliert. 
Eine Betrachtung des alternativen Indikators SchulabgÃ¤nger mit Hochschulreife zeigt, dass dieser besser mit dem Anteil BeschÃ¤ftigter mit akademischem Bildungsabschluss korreliert, aber dafÃ¼r recht stark negativ mit dem Anteil der BeschÃ¤ftigten ohne Abschluss. Das kann daran liegen, dass dort wo der Anteil von Personen mit akademischem Abschluss hoch ist, auch der Druck fÃ¼r SchulabgÃ¤nger einen Abschluss zu machen geringer ist.


## III.Imputation fehlender Werte

Das bisherige Imputationsmodell nutzt Arbeitslosigkeit als PrÃ¤diktoren. In den Daten fÃ¼r 2016 fehlen fÃ¼r diese Variable 6 Werte.
Der Einfachheit halber werden diese interpoliert. 

```{r Vereinzelte Missings auf den Imputationsvariablen interpolieren}
# Anzahl der Missings fÃ¼r die Indikatoren
...
# FÃ¤lle betrachten: Beispiel 13075152 
...
# FÃ¤lle betrachten: 16063104
TimeSeries_for_Missing <- Workfile %>%  filter(Gemeindekennziffer==16063104) %>% arrange(Gemeindekennziffer, Jahr, Bevoelkerung)
TimeSeries_for_Missing

#Aufgrund der variierenden VerfÃ¼barkeit von Daten und der Unklarheit Ã¼ber die Indikatoren auf Gemeindeebene, werden die Gemeinden vorerst ausgeschlossen.
...
# Interpolation der fehlenden Werte Ã¼ber die Zeitreihe (Mittelwert: Vorjahr, Nachjahr)
...
#Erneut Anzahl der Missings fÃ¼r die Indikatoren
missings_table = as.data.frame(expand.grid("Jahr"=1998:max(Basedata$Jahr)))
predictors_list = data.frame(Variable=character(), Missings=double(), stringsAsFactors = FALSE)
for (column in level_table[,1]){
  for (year in 1998:max(Basedata$Jahr)){
...
# vor allem auf der Variablen SchulabgÃ¤nger mit Hochschulreife gibt es ebenfalls viele 0-Werte,
# Es ist mÃ¶glich, dass es in manchen Kreisen keine Schulen mit gymnasialer Oberstufe gibt, 
...
########Imputation fÃ¼r SchulabgÃ¤nger mit Hochschulreife
# Die SchulabgÃ¤nger mit Hochschulreife werden separat imputiert und durch alle Kovariaten informiert
# (@Marvin: Eine Korrektur fÃ¼r G8 wird hier nicht vorgenommen. Es geht nur darum die Missings zu imputieren )
...
# Percentile fÃ¼r ausgewÃ¤hlte Variablen bilden
# 
...
predictors_list = predictors_list[order(predictors_list$Missings),]
predictors_list
#Ein Missing auf Arbeitslosigkeit -> Identifikation
...
TimeSeries_for_Missing
#UnerklÃ¤rliches Missing/NA auf Arbeitslosigkeit in 2015 -> Vorerst Ausschluss
...
# Die Bildungsvariablen Anteil der SchulabgÃ¤nger mit Hochschulreife und Anteil der SchulabgÃ¤nger ohne Abschluss werden als Quote relativ zur Gesamtanzahl der SchulabgÃ¤nger berechnet. Durch die G8-Schulreformen und ihre ZurÃ¼cknahme ergeben sich deshalb in den BundeslÃ¤ndern zu verschiedenen Zeitpunkten Verzerrungen. 
# Diese Verzerrung wird Ã¼ber eine Regressionsmodell herausgerechnet.

# Generierung der Variablen zur Identifikation der Reformen (G8), RÃ¼ckker zu G9 (SN_KA) und abweichender Anerkennung von AbschlÃ¼ssen fÃ¼r GymnasiastInnen (THvor2004).
Impdata.imputed <- Impdata.imputed %>%
...
# Anpassung1: Ersetzen der Werte in den durch Verzerrungen betroffene FÃ¤lle durch Prediction aus Kovariaten und Zeitreihe.
...
# Anpassung2: Ersetzen der Werte in den von Verzerrungen betroffenen FÃ¤lle durch um Reformeffekte bereinigte Quoten.
...
#
#summary(ow_modell)
#summary(ow_modell)$coefficients[23,1]
#
#
#ow_modell <- lm(BeschaeftigtemitakadAbschluss ~ Jahr_Dummy + relevel(Jahr_Dummy, ref = "2012") * OW, data = #dat_ow_modell,na.action="na.exclude")
#
#summary(ow_modell)
#summary(ow_modell)$coefficients[23,1]
...
```{r Deckelung der BeschÃ¤ftigtenquote}
...
```{r}
# Stata-Datensatz rausschreiben
stata_data <- Impdata.imputed %>% mutate(SchulabgmitAbi_adj = SchulabgaengermitHochschulreife_adj) %>% select(-SchulabgaengermitHochschulreife_mr, -SchulabgaengermitHochschulreife_adj)
write_dta(stata_data, paste0("Outfiles/2022b/Stata/Impdata_check_2019.dta"))

# RDS-Datensatz rausschreiben
write_rds(Impdata.imputed, paste0("Outfiles/2022b/Impdata_check.rds"))
...
# Es gibt eine bimodale Verteilung bei den BeschÃ¤ftigten ohne Abschluss, die in Ost- und Westdeutschland jedoch unimodal ist
```

## IV. Faktorenanalyse (Hauptkomponentenanalyse) inklusive Generierung der Faktorscores
```{r Tibbles fÃ¼r die Teilscores generieren, echo=FALSE}

# Variablenliste fÃ¼r die Faktorenanalyse 
print(listofdeterminants)
TS_Arbeitswelt <- Impdata.imputed  %>% ungroup() %>% select(Beschaeftigtenquote,Arbeitslosigkeit,Bruttoverdienst_ln)
...
TS_Einkommen_adj   <- Impdata.imputed %>% select(Einkommensteuer_ln,Haushaltseinkommen_ln,Schuldnerquote) 
# fÃ¼r den Vergleich der Ergebnisse wird zunÃ¤chst ein Datensatz fÃ¼r die Variablenauswahl der Revision 2019 generiert
TS_Bildung_adj <- Impdata.imputed %>% select(BeschaeftigtemitakadAbschluss,BeschaeftigteohneAbschluss_adj,SchulabgaengerohneAbschluss_adj) 

# Check dieser LÃ¶sung fÃ¼r das 2014er Sample 
TS_Bildung_r2014 <- Impdata.imputed %>% filter(Jahr<2015) %>%  select(BeschaeftigtemitakadAbschluss,BeschaeftigteohneAbschluss_adj,SchulabgaengerohneAbschluss) 

TS_Bildung_4items <- Impdata.imputed %>% select(BeschaeftigtemitakadAbschluss,BeschaeftigteohneAbschluss_adj,SchulabgaengerohneAbschluss, SchulabgaengermitHochschulreife) 

TS_Bildung_4items_without_BoA <- Impdata.imputed %>% select(BeschaeftigtemitakadAbschluss,SchulabgaengerohneAbschluss, SchulabgaengermitHochschulreife) 

```

# Faktorenanalyse basierend auf Hauptkomponentenanalyse fÃ¼r jede der drei Subscalen
```{r PCA fÃ¼r die Teilscores, echo=FALSE}


# PCA fÃ¼r die Arbeitsweltdimension
...
# PCA fÃ¼r die Einkommensdimension
...
# PCA fÃ¼r die Bildungsdimension
...
# die Faktorladung fÃ¼r SchulabgaengerohneAbschluss ist auf dem zweiten Faktor schwach
...
# Check der Bildungskomponente in Revision 2018 (Daten fÃ¼r 2014)
...
# fÃ¼r den JoHM Beitrag wird die Bildungsdimension mit den zwei Indikatoren generiert
...
Resultdataset$TS_Bildung_adj <- as.numeric(((Resultdataset$BeschaeftigtemitakadAbschluss*-0.66)+(Resultdataset$SchulabgaengerohneAbschluss_adj*0.33))/2)
...
write_rds(Resultdataset, paste0("Outfiles/2022b/Resultdataset.rds"))
write_dta(Resultdataset, paste0("Outfiles/2022b/Stata/Resultdataset.dta"))
```
...
for(mykennziffer in exportlist$Kennziffern) {
  ...
  dir.create("Outfiles/2022b/", showWarnings=F)
  dir.create("Outfiles/2022b/Bund/", showWarnings=F)  
  dir.create(paste0("Outfiles/2022b/Bund/",mylabel), showWarnings=F)
  mydata <- outputdata.agg %>% ungroup() %>% select(mykennziffer, GISD_Score, GISD_5, GISD_10, GISD_k, myname, Jahr)
  write.csv(mydata, paste0("Outfiles/2022b/Bund/",mylabel,"/",mylabel,".csv"))
  ...
  write_dta(mydata, paste0("Outfiles/2022b/Bund/",mylabel,"/",mylabel,"_long.dta"))
  ...
  # Ausgabe BundeslÃ¤nder
  ListeBula <- unique(outputdata.bula$Bundesland)
  dir.create("Outfiles/2022b/Bundesland", showWarnings=F)  
  for(myland in ListeBula) {
  dir.create( paste0("Outfiles/2022b/Bundesland/",myland), showWarnings=F)  
    dir.create( paste0("Outfiles/2022b/Bundesland/",myland,"/",mylabel), showWarnings=F)  
    mydata.bula <- outputdata.bula %>% filter(Bundesland==myland) %>% ungroup() %>% select(GISD_Score, mykennziffer, myname, Jahr)
    write.csv(mydata.bula, paste0("Outfiles/2022b/Bundesland/",myland,"/",mylabel,"/",mylabel,".csv"))
    ...
    #write_dta(mydata.bula, paste0("Outfiles/2022b/Bundesland/",myland,"/",mylabel,"/",mylabel,".dta"))
  }
  }  
}
...
for (mykennziffer in c("PLZ2","PLZ3","PLZ4","PLZ5")) {
  ...
  outputdata <- outputdata %>% group_by_at(vars("JAHR",mykennziffer)) %>% 
    summarise(GISD_Score = weighted.mean(GISD_Score,EW_Area), BevÃ¶lkerung = sum(EW_Area)) %>%
    group_by(JAHR)
  ...
  dir.create( paste0("Outfiles/2022b"), showWarnings=F) 
  dir.create( paste0("Outfiles/2022b/Bund/"), showWarnings=F) 
  dir.create( paste0("Outfiles/2022b/Bund/",mylabel), showWarnings=F) 
  mydata <- outputdata %>% ungroup() 
  write.csv2(mydata, paste0("Outfiles/2022b/Bund/",mylabel,"/",mylabel,".csv"))
  mydata <- outputdata %>% ungroup() 
  names(mydata) <- gsub("\\.","_",make.names(names(mydata)))
  names(mydata) <- gsub("\\?","oe",names(mydata))
  names(mydata) <- gsub("\\?","ae",names(mydata))
...
Es gibt andererseits MÃ¶glichkeiten den GISD weiter zu verbessern. Einzelne Schwachstellen sollen hier kurz erwdÃ¤hnt werden.
...
# Hier wird eine Faktoranalyse fÃ¼r die Bildungskomponente durchgefÃ¼hrt, ohne die
# BeschÃ¤ftigung ohne Abschluss zu verwenden, welche sich systematisch zwischen
# West und Ost unterscheidet
TS_Bildung_4items_without_BoA <- Impdata.imputed %>% dplyr::select(BeschaeftigtemitakadAbschluss,SchulabgaengerohneAbschluss_adj, SchulabgaengermitHochschulreife_adj) 
...
# Danach muss der Code oben beginnend beim Datenexport angewandt werden (Achtung! Oben erstellte Exporte werden dann Ã¼berschrieben)
# DafÃ¼r wird das Working Directory temporÃ¤r geÃ¤ndert
...
```{r eval=FALSE, include=FALSE}
# Das working directory wieder zurÃ¼cksetzen
setwd("..") 
getwd()
