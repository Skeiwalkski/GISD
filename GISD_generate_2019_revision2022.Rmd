...
adj_G8_jahr <- function(data,outcome_name){
  ...
  print(mymodell2)
    mydata <- mydata %>% select(Outcome) %>% 
      mutate(Adjusted = residuals(mymodell2,newdata=mydata)) %>%
      mutate(Outcome = Adjusted) %>% pull(Outcome)
}
...
Messaenderung_Besch <- function(data,outcome_name){
  ...
  print(mymodell_Messaenderung)  
    mydata <- mydata %>% select(Outcome) %>%
      mutate(Adjusted= residuals(mymodell_Messaenderung,newdata = mydata)) %>%
      mutate(Outcome = Adjusted) %>%
      pull(Outcome)
}
