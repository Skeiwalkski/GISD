...
inputdataset <- list.files("Data/Arbeitslosigkeit_Kreisbene/")
for(file in inputdataset){
  suppressMessages(myimport <- read_excel(paste0("Data/Arbeitslosigkeit_Kreisbene/",file), skip = 1, sheet = "Daten"))
  names(myimport)[1] <- "Kennziffer"
  myimport[2:3] <- NULL
  myimport <- myimport %>% gather(key = "Jahr", value = "Value" , -"Kennziffer", convert=T, na.rm = T) %>%
    mutate(Kennziffer=as.numeric(as.character(Kennziffer)), Value=as.numeric(Value))
  names(myimport)[3] <- strsplit(strsplit(file,"_")[[1]][2],"[.]")[[1]][1]
  Workfile <- full_join(Basedata, myimport, by=c("Kennziffer", "Jahr"))
}
rm(myimport)
...
```{r}
# Stata-Datensatz rausschreiben
stata_data <- Impdata.imputed %>% mutate(SchulabgmitAbi_adj = SchulabgaengermitHochschulreife_adj,BeschaeftigtemitakadAbschlussadj = BeschaeftigtemitakadAbschluss_adj) %>% select(-SchulabgaengermitHochschulreife_adj, -BeschaeftigtemitakadAbschluss_adj)
