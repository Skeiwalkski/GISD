...
dir.create("Outfiles/2021", showWarnings=T)
dir.create("Outfiles/2021/Bund", showWarnings=T)
dir.create("Outfiles/2021/Other", showWarnings=T)
dir.create("Outfiles/2021/Stata", showWarnings=T)
setwd(home)
getwd()
...
Gemeinden_INKAR <- load_dataset("Gemeinden-GVB") %>% 
  na.omit() %>%
  mutate(Kennziffer=as.numeric(gem19),"Kennziffer Gemeindeverband"=gvb19, fl19=as.numeric(fl19))
# Pipes: 
...
Gemeindeverbaende_INKAR <- load_dataset("Gemeindeverbände") %>% 
  na.omit() %>% 
  select("Kennziffer Gemeindeverband"=gvb19,"Name des Gemeindeverbands"=gvb19name)
...
id_dataset <- Gemeinden_INKAR %>% 
              select(Gemeindekennziffer=Kennziffer,"Name der Gemeinde"=gem19name,"Kennziffer Gemeindeverband", "Bevölkerung") %>% 
...
ind_level <- c("Gemeindeverband","Gemeindeverband","Kreis", "Gemeindeverband", "Kreis", "Kreis", "Kreis", "Kreis", "Gemeindeverband", "Kreis", "Kreis")
...
Basedata_Kreisebene <- Basedata %>% select(krs15=Kennziffer,Jahr,listofdeterminants) %>% 
  select(-Arbeitslosigkeit,-Einkommensteuer,-Beschaeftigtenquote, -ErwerbsfaehigeBevoelkerung) %>% rename(Kreis=krs15)
...
Workfile <- as.data.frame(expand.grid("Kennziffer"=Gemeinden_INKAR %>% pull(Kennziffer),"Jahr"=seq(min(Basedata$Jahr):max(Basedata$Jahr)) + min(Basedata$Jahr)-1)) %>% mutate(Kreiskennziffer=floor(as.numeric(Kennziffer)/1000)) %>% as_tibble() %>%
   ...
   select(Gemeindekennziffer="Kennziffer",Kreis="Kreiskennziffer",Gemeindeverband="Kennziffer Gemeindeverband",Jahr,Bevoelkerung="Bevölkerung") %>% mutate(Gemeindeverband=as.numeric(Gemeindeverband), Bevoelkerung=as.numeric(Bevoelkerung)) %>% 
...
Workfile <- Workfile %>% mutate(Arbeitslosigkeit = Arbeitslosigkeit / ErwerbsfaehigeBevoelkerung * 1000, Arbeitslosigkeit=ifelse(is.nan(Arbeitslosigkeit),NA,Arbeitslosigkeit)) %>%
...
Workfile <- Workfile %>%  group_by(Gemeindeverband) %>% 
  mutate(impu_arblos = ifelse(Bevoelkerung>0,na.approx(Arbeitslosigkeit),Arbeitslosigkeit), 
         impu_mA = ifelse(Bevoelkerung>0,na.approx(SchulabgaengermitHochschulreife),SchulabgaengermitHochschulreife))  %>% 
  select(-Arbeitslosigkeit, 
         -SchulabgaengermitHochschulreife) %>% 
  rename(Arbeitslosigkeit=impu_arblos, 
         SchulabgaengermitHochschulreife=impu_mA) %>% 
  ungroup()
...
TimeSeries <- Workfile %>%  filter(Gemeindekennziffer==13075152) %>% select(Gemeindekennziffer, Jahr, listofdeterminants) %>% arrange(Gemeindekennziffer, Jahr)
TimeSeries
...
my_ts_imputer <- function(data,outcome_name){
  ...
  mymodell <- lm(Outcome ~ I(Jahr*Jahr*MEAN) + I(Jahr*MEAN) + Arbeitslosigkeit + SchulabgaengerohneAbschluss, data = mydata  , na.action="na.exclude")
...
Impdata.imputed <- Impdata %>%
  mutate(Beschaeftigtenquote=ifelse(Bevoelkerung>0,my_ts_imputer(.,"Beschaeftigtenquote"),Beschaeftigtenquote),
...
summary(ow_modell)
summary(ow_modell)$coefficients[21,1]
...
OW <- function(data,outcome_name){
  ...
  print(mymodell_ow)  
    mydata %>% mutate(coef = summary(mymodell_ow)$coefficients[21,1]) %>%
...
write_rds(Impdata.imputed, paste0("Outfiles/Impdata_check.rds"))
...
TS_Arbeitswelt <- Impdata.imputed  %>% ungroup() %>% dplyr::select(Beschaeftigtenquote,Arbeitslosigkeit,Bruttoverdienst_ln)
TS_Arbeitswelt_adj <- Impdata.imputed  %>% ungroup() %>% dplyr::select(Beschaeftigtenquote_adj,Arbeitslosigkeit,Bruttoverdienst_ln) 
TS_Einkommen   <- Impdata.imputed %>% dplyr::select(Einkommensteuer,Haushaltseinkommen,Schuldnerquote) 
TS_Einkommen_adj   <- Impdata.imputed %>% dplyr::select(Einkommensteuer_ln,Haushaltseinkommen_ln,Schuldnerquote) 
# für den Vergleich der Ergebnisse wird zunächst ein Datensatz für die Variablenauswahl der Revision 2019 generiert
TS_Bildung_adj <- Impdata.imputed %>% dplyr::select(BeschaeftigtemitakadAbschluss,BeschaeftigteohneAbschluss_adj,SchulabgaengerohneAbschluss_adj) 
...
save(GISD_Komponents, file="Outfiles/2021/GISD_Komponents.RData")
...
write_rds(Resultdataset, paste0("Outfiles/Resultdataset.rds"))
```
...
for(mykennziffer in exportlist$Kennziffern) {
  ...
  dir.create("Outfiles/2021_v2/", showWarnings=F)
  dir.create("Outfiles/2021_v2/Bund/", showWarnings=F)  
  dir.create(paste0("Outfiles/2021_v2/Bund/",mylabel), showWarnings=F)
  mydata <- outputdata.agg %>% ungroup() %>% select(mykennziffer, GISD_Score, GISD_5, GISD_10, GISD_k, myname, Jahr)
  write.csv(mydata, paste0("Outfiles/2021_v2/Bund/",mylabel,"/",mylabel,".csv"))
  ...
  write_dta(mydata, paste0("Outfiles/2021_v2/Bund/",mylabel,"/",mylabel,"_long.dta"))
  ...
  dir.create("Outfiles/2021_v2/Bundesland", showWarnings=F)  
  for(myland in ListeBula) {
  dir.create( paste0("Outfiles/2021_v2/Bundesland/",myland), showWarnings=F)  
    dir.create( paste0("Outfiles/2021_v2/Bundesland/",myland,"/",mylabel), showWarnings=F)  
    mydata.bula <- outputdata.bula %>% filter(Bundesland==myland) %>% ungroup() %>% select(GISD_Score, mykennziffer, myname, Jahr)
    write.csv(mydata.bula, paste0("Outfiles/2021_v2/Bundesland/",myland,"/",mylabel,"/",mylabel,".csv"))
    ...
    write_dta(mydata.bula, paste0("Outfiles/2021_v2/Bundesland/",myland,"/",mylabel,"/",mylabel,".dta"))
  }
  }  
}
...
for (mykennziffer in c("PLZ2","PLZ3","PLZ4","PLZ5")) {
  ...
  dir.create( paste0("Outfiles/2021_v2"), showWarnings=F) 
  dir.create( paste0("Outfiles/2021_v2/Bund/"), showWarnings=F) 
  dir.create( paste0("Outfiles/2021_v2/Bund/",mylabel), showWarnings=F) 
  mydata <- outputdata %>% ungroup() 
  write.csv2(mydata, paste0("Outfiles/2021_v2/Bund/",mylabel,"/",mylabel,".csv"))
  mydata <- outputdata %>% ungroup() 
  names(mydata) <- gsub("\\.","_",make.names(names(mydata)))
  names(mydata) <- gsub("\\?","oe",names(mydata))
  names(mydata) <- gsub("\\?","ae",names(mydata))
