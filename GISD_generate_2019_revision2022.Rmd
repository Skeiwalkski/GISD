...
An dieser Stelle werden auch Quoten für einige Indikatoren berechnet, die nur in absoluten Zahlen vorliegen: Arbeitslosen pro 1000 Einwohner im Erwerbsalter, Anteil der SV Beschäftigte ohne Abschluss sowie der Anteil der SV Beschäftigten mit akad. Abschluss an allen SV Beschäftigten.
...
inputdataset <- inputdataset[-c(5,11)];  
...
inputdataset <- list.files("Data/Indikatoren_Kreisbene/")

for(file in inputdataset){
  suppressMessages(myimport <- read_excel(paste0("Data/Indikatoren_Kreisbene/",file), skip = 1, sheet = "Daten"))
  names(myimport)[1] <- "Kennziffer"
  myimport[2:3] <- NULL
  myimport <- myimport %>% gather(key = "Jahr", value = "Value" , -"Kennziffer", convert=T, na.rm = T) %>%
    mutate(Kennziffer=as.numeric(as.character(Kennziffer)), Value=as.numeric(Value))
