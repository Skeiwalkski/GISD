...
```{r}
Lebenserwartung_GISD <- Lebenserwartung_female_mean %>% select(-Lebenserwartung_adj) %>% rename(LE_Mean_Female = Lebenserwartung_mean) %>% left_join(Lebenserwartung_male_mean, by = c("Kreis", "Jahr", "GISD_5") )%>% select(-Lebenserwartung_adj) %>% rename(LE_Mean_Male = Lebenserwartung_mean) %>% distinct()

write.csv(Lebenserwartung_GISD, "P:/LE_Kreise_GISD.csv")
```