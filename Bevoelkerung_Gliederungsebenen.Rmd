...
destatis <- read_xlsx("Datea/Referenz/") %>% mutate(Gemeindekennziffer = as.numeric(paste0(...3,...4,...5,...7)), Kreiskennziffer = as.numeric(paste0(...3,...4,...5)), Bevölkerung = as.numeric(...10), Gemeindename = ...8) %>% filter(Bevölkerung != is.na(NA)) %>% select(Gemeindekennziffer, Kreiskennziffer, Bevölkerung, Gemeindename)
